...
```bash
PYTHONPATH=src python3 -m ai_dev_review scan [paths...]
...
- Always: preserve original transcript files; write generated state under
  `.ai-review/` unless configured otherwise; make scanner re-runnable.
...
- A new `ai-dev-review/` directory exists with a runnable CLI project.
